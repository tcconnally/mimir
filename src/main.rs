        ...
        Some(Commands::Serve {
            ...
            if let Some(mode) = tmode {
                ...
                let rt = match tokio::runtime::Runtime::new() {
                    Ok(rt) => rt,
                    Err(e) => {
                        eprintln!("mimir: failed to create async runtime: {}", e);
                        std::process::exit(1);
                    }
                };
                rt.block_on(async {
                    let listener = match tokio::net::TcpListener::bind(&transport_addr).await {
                        Ok(l) => l,
                        Err(e) => {
                            eprintln!("mimir: failed to bind to {}: {}", transport_addr, e);
                            std::process::exit(1);
                        }
                    };
                    if let Err(e) = axum::serve(listener, transport_router).await {
                        eprintln!("mimir: server error: {}", e);
                        std::process::exit(1);
                    }
                });
            } else {
